...
function GrandTotal(){
  ...
  $(TotalPriceArr).each(function(){
    TotalValue += parseFloat($(this).text().replace(/,/g, "").replace("RM",""));
  });
  ...
  if(discount != null){
    ...
    $("#totalValue").text(accounting.formatMoney(f_discount,{symbol:"RM",format: "%s %v"}));
    $("#totalValue1").text(accounting.formatMoney(TotalValue,{format: "%v"}));
  }else{
    $("#totalValue").text(accounting.formatMoney(TotalValue,{symbol:"RM",format: "%s %v"}));
    $("#totalValue1").text(accounting.formatMoney(TotalValue,{format: "%v"}));
  }
};
        ...
        title: "Enter number of items:",
		  ...
		  .then((value) => {
            ...
            swal("Error","Please enter a number!","error");
    				...
    				$('#tableData').append("<tr class='prd'><td class='barcode text-center'>"+barcode+"</td><td class='text-center'>"+product+"</td><td class='price text-center'>"+accounting.formatMoney(price,{symbol:"RM",format: "%s %v"})+"</td><td class='text-center'>"+unit+"</td><td class='qty text-center'>"+value+"</td><td class='totalPrice text-center'>"+accounting.formatMoney(total,{symbol:"RM",format: "%s %v"})+"</td><td class='text-center p-1'><button class='btn btn-danger btn-sm' type='button' id='delete-row'><i class='fas fa-times-circle'></i></button><tr>");
	          GrandTotal();
        }
			}
  });
...
$(document).on('click','.Enter',function(){
  ...
  if (TotalPriceArr == 0){
    ...
    $('.price').each(function(){
      price.push($(this).text().replace(/,/g, "").replace("RM",""));
    });
    ...
    .then((value) => {  
      if(value == "") {
        ...
        if(isNaN(qtynum)){
          ...
          var change = 0;
          var TotalValue = parseFloat($('#totalValue').text().replace(/,/g, "").replace("RM",""));
          var cashAmount = parseFloat(qtynum);

          if(isNaN(TotalValue) || isNaN(cashAmount)) {
            swal("Error", "Invalid amount", "error");
            return;
          }

          if(cashAmount < TotalValue){
            swal("Error","Insufficient cash amount","error");
          }else{
            change = cashAmount - TotalValue;
            ...
            $.ajax({
              ...
              success: function(data){
                if(data === "success"){
                  swal({
                    title: "Change is " + accounting.formatMoney(change, {symbol:"RM", format: "%s %v"}),
                    icon: "success",
                    buttons: "Okay",
                  })
                  .then((okay) => {
