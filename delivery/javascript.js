...
$(document).ready(function(){
	...
	$(document).on('click','.remove_row',function(){
		...
		var final_amount = $('#final_total_amount').text().replace("RM","");
		...
		if(isNaN(result_amount)){
			...
			if(total_product_amount == ""){
				...
				$('#total_amount').text('RM '+minus_total);
			}else{
				$('#final_total_amount').text('RM 0.00');
			}
			
		}else{
			$('#final_total_amount').text('RM '+result_amount);
		}
	...
	function final_total(count){
		...
		$('#final_total_amount').text('RM '+final_product_amount);
	}
